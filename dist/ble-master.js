...
class BLEMaster {
    ...
    get get() {
        if (!this.#get_helper) { // cache
            this.#get_helper = new Get(this.#getDevices);
        }
...
*/
     ...
     */
    startScan(response_callback, options = {}) { // note as ble-master is now executed as a bg service, I repopulate the devices list during connection procedure. To clean !!!
    ...
    connect(dev_addr, response_callback) {
        ...
        if (dev_addr.includes('xx')) { // @add 1.5.7
            ...
            if (!matched_dev_addr) {
                debugLog(1, ERR.DEVICE_NOT_FOUND, dev_addr, " on connect");
        ...
        if (this.get.isConnected(dev_addr)) { // @add 1.2.3
    ...
    startListener(profile_object, dev_addr, response_callback) {
        ...
        if (this.#devices[dev_addr].profile_pid) {
            debugLog(2, `Listener already active for ${dev_addr}. Stopping the existing listener.`);
     ...
     */
	generateProfileObject(services,dev_addr, permissions = {}) {
     ...
     */
    quit(dev_addr) { // watching this: might need to bring back the "disconnect all" solution
        ...
        this.stopListener(dev_addr); // Stop the listener for the device
...
// note to fix : chara is null -> that's an issue
    ...
    #waitForWriteCompletion(callback, operation_type) {
        ...
        const cb_check_completion = () => {
            ...
            const device = this.#getCurrentDevice();
            if (device && device.profile_pid) {
                const dev_addr = device.dev_addr;
               // console.log("BLK check writeCompletion, mac is: ", dev_addr); // J'ai un doute que #getCurrentDevice() renvoie le bon device
                const on_instance = this.#onInstance;
                if (on_instance && (on_instance._getDescWriteCompleteFlag() || on_instance._getCharaWriteCompleteFlag())) {
                    // reset flags for next operation
                    on_instance._setDescWriteCompleteFlag(false);
                    on_instance._setCharaWriteCompleteFlag(false);
    ...
    #waitForReadCompletion(is_descriptor, callback) {
        ...
        const cb_check_completion = () => {
            ...
            if (device && device.profile_pid) {
                const dev_addr = device.dev_addr;
                if (!this.#onInstance) {
                    debugLog(1, "On instance is not available");
                    callback();
                    return;
                }
                const on_instance = this.#onInstance;
                if (on_instance && ((is_descriptor && on_instance._getDescReadCompleteFlag()) || (!is_descriptor && on_instance._getCharaReadCompleteFlag()))) {
                    on_instance._setDescReadCompleteFlag(false);
...
class On {
    ...
    // Write flags 
    #chara_write_complete_flag = false;
    #desc_write_complete_flag = false;
    #last_chara_write_status = 0;
    #last_desc_write_status = 0;

    // Read flags 
    #chara_read_complete_flag = false;
    #last_chara_read_status = 0;
    #desc_read_complete_flag = false;
    #last_desc_read_status = 0;

    // callbacks storage for dereg (no setters/getters. internal use only)
    _cb_charaReadComplete = null;
    _cb_charaValueArrived = null;
    _cb_charaWriteComplete = null;
    _cb_descReadComplete = null;
    _cb_descValueArrived = null;
    _cb_descWriteComplete = null;
    _cb_charaNotification = null;
    _cb_serviceChangeBegin = null;
    _cb_serviceChangeEnd = null;

    constructor(profile_pid) {
        this.#profile_pid = profile_pid;
    }

    _setCharaWriteCompleteFlag(value) { this.#chara_write_complete_flag = value; }
    _setDescWriteCompleteFlag(value) { this.#desc_write_complete_flag = value; }
    _getCharaWriteCompleteFlag() { return this.#chara_write_complete_flag; }
    _getDescWriteCompleteFlag() { return this.#desc_write_complete_flag; }
    _getLastCharaWriteStatus() { return this.#last_chara_write_status; }
    _getLastDescWriteStatus() { return this.#last_desc_write_status; }

    _setDescReadCompleteFlag(value) {  this.#desc_read_complete_flag = value;  }
    _getDescReadCompleteFlag() {  return this.#desc_read_complete_flag;  }
    _getLastDescReadStatus() {  return this.#last_desc_read_status;  }
    _setCharaReadCompleteFlag(value) { this.#chara_read_complete_flag = value; }
    _getCharaReadCompleteFlag() { return this.#chara_read_complete_flag; }
    _getLastCharaReadStatus() { return this.#last_chara_read_status; }
    ...
    charaReadComplete(callback) {
        ...
        this._cb_charaReadComplete = callback;
        
        hmBle.mstOnCharaReadComplete((response) => {
            const { profile, uuid, status } = response;
            if (this.#profile_pid === profile) {
                this._setCharaReadCompleteFlag(true);
                this.#last_chara_read_status = status;

                if (this._cb_charaReadComplete)
                    this._cb_charaReadComplete(uuid, status);
                else
    ...
    charaValueArrived(callback) {
        ...
        this._cb_charaValueArrived = callback;

        hmBle.mstOnCharaValueArrived((response) => {
            const { profile, uuid, data, length } = response;
            if (this.#profile_pid === profile) {
                this._setCharaReadCompleteFlag(true);  // make sure we aren't required to sub to charaReadComplete
                this.#last_chara_read_status = 0;
                
                if (this._cb_charaValueArrived)
                    this._cb_charaValueArrived(uuid, data, length);
                else
    ...
    charaWriteComplete(callback) {
        ...
        this._cb_charaWriteComplete = callback;

        hmBle.mstOnCharaWriteComplete((response) => {
            if (this.#profile_pid === response.profile) {
                this._setCharaWriteCompleteFlag(true);

                if (this._cb_charaWriteComplete)
                    this._cb_charaWriteComplete(response.uuid, response.status);
                else
    ...
    descReadComplete(callback) {
        ...
        this._cb_descReadComplete = callback;

        hmBle.mstOnDescReadComplete((response) => {
            const { profile, chara, desc, status } = response;

            if (this.#profile_pid === profile) {
                this._setDescReadCompleteFlag(true); // set the flag
                this.#last_desc_read_status = status;

                if (this._cb_descReadComplete)
                    this._cb_descReadComplete(chara, desc, status);
                else
    ...
    descValueArrived(callback) {
        ...
        this._cb_descValueArrived = callback;
        ...
        hmBle.mstOnDescValueArrived((response) => {
            ...
            if (this.#profile_pid === profile) {
                ...
                this._setDescReadCompleteFlag(true); // @add 1.4.7 
                this.#last_desc_read_status = 0;

                if (this._cb_descValueArrived)
                    this._cb_descValueArrived(chara, desc, data, length);
                else
    ...
    descWriteComplete(callback) {
        ...
        this._cb_descWriteComplete = callback;

        hmBle.mstOnDescWriteComplete((response) => {
            const { profile, chara, desc, status } = response;

            if (this.#profile_pid === profile) {
                this._setDescWriteCompleteFlag(true); // set the flag
                this.#last_desc_write_status = status;

                if (this._cb_descWriteComplete)
                    this._cb_descWriteComplete(chara, desc, status);
                else
    ...
    charaNotification(callback) {
        ...
        this._cb_charaNotification = callback;
        ...
        hmBle.mstOnCharaNotification((response) => {
         ...
         //   if (this.#profile_pid === profile) {
           //     console.log(`Notification received for profile_pid: ${profile}, uuid: ${uuid}`);
                ...
                debugLog(2, `Notification ignored for mismatched profile_pid: ${profile}`);
            }*/
        });
    }
    ...
    serviceChangeBegin(callback) {
        ...
        this._cb_serviceChangeBegin = callback;

        hmBle.mstOnServiceChangeBegin((response) => {
            const { profile } = response;

            if (this.#profile_pid === profile) {
                if (this._cb_serviceChangeBegin)
                    this._cb_serviceChangeBegin();
                else
    ...
    serviceChangeEnd(callback) {
        ...
        this._cb_serviceChangeEnd = callback;

        hmBle.mstOnServiceChangeEnd((response) => {
            const { profile } = response;
            
            if (this.#profile_pid === profile) {
                if (this._cb_serviceChangeEnd)
                    this._cb_serviceChangeEnd();
                else
...
class Off {
    ...
    constructor(on_instance) {
        this.#on_instance = on_instance;
    }
    /**
     ...
     */
    charaReadComplete() {
        this.#on_instance._cb_charaReadComplete = null;
    }
     ...
     */
    charaValueArrived() {
        this.#on_instance._cb_charaValueArrived = null;
    }
     ...
     */
    charaWriteComplete() {
        this.#on_instance._cb_charaWriteComplete = null;
    }
     ...
     */
    descReadComplete() {
        this.#on_instance._cb_descReadComplete = null;
    }
     ...
     */
    descValueArrived() {
        this.#on_instance._cb_descValueArrived = null;
    }
     ...
     */
    descWriteComplete() {
        this.#on_instance._cb_descWriteComplete = null;
    }
     ...
     */
    charaNotification() {
        this.#on_instance._cb_charaNotification = null;
    }
     ...
     */
    serviceChangeBegin() {
        this.#on_instance._cb_serviceChangeBegin = null;
    }
     ...
     */
    serviceChangeEnd() {
        this.#on_instance._cb_serviceChangeEnd = null;
    }
 ...
 *      - 2A05: Service Changed Characteristic (should be handled) *  - Reliable Write Operations *  - Long Chara Values *      - due to a relatively small MTU it shoudn't be possible to handle charas bigger than MTU packet *  - Queue
